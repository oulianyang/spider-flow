...
var version = 'lastest';
/**
 * 读取当前页面 URL 中除系统参数（id）外的所有查询参数，
 * 用于外部项目通过 iframe 嵌入时传递参数给爬虫流程。
 * 例：editor.html?id=xxx&keyword=手机&page=2  →  {keyword: '手机', page: '2'}
 */
...
$(function(){
	...
	function loadShapes(editor,container){
		...
		// 如果 URL 包含 callback 参数，添加"回调请求"节点
		var urlParams = getUrlParameters();
		if(urlParams['callback']){
			shapes.splice(shapes.length - 1, 0, {
				name : 'callbackRequest',
				image : 'data:image/png;base64,',
				title : '回调请求',
				desc : '向回调地址发送HTTP请求。<br/>URL默认取REST API/iframe传入的callback参数。<br/>支持方法参考命令提示。'
			});
		}
...
//		editor.onSelectedCell();
		...
		Save();
	}).on('click','.btn-cron',function(){
		openCronDialog();
		}).on('click','.btn-log',function(){
			var id = getQueryString('id') || flowId;
			if (!id) {
				layui.layer.msg('请先保存流程');
				return;
			}
			layui.layer.open({
				type: 1,
				title: '执行日志',
				content: '<iframe src="log.html?id=' + id + '" style="width:100%;height:100%;border:none;display:block;"></iframe>',
				area: ['900px', '550px'],
				shade: 0,
				maxmin: true,
				resize: false
...
function Save(){
	validXML(function(){
		$.ajax({
			...
			success : function(id) {
				flowId = id;
				saveCronConfigChain(id, function() {
...
function openCronDialog() {
	...
	$.ajax({
		...
		success: function(flow) {
			var currentCron = flow.cron || '';
			var currentEnabled = (flow.enabled === '1');
			if (pendingCronConfig) {
				if (pendingCronConfig.cron !== null) currentCron = pendingCronConfig.cron;
				if (pendingCronConfig.enabled !== null) currentEnabled = pendingCronConfig.enabled;
			}
			layui.layer.open({
				type: 1,
				title: '定时任务配置',
				area: ['880px', '750px'],
				content: '<iframe id="cronEditorFrame"'
					+ ' src="editCron.html?id=' + id + '&cron=' + encodeURIComponent(currentCron) + '&picker=1"'
					+ ' style="width:850px;height:620px;border:1px solid #e6e6e6;display:block;margin:0 auto;">'
					+ '</iframe>'
					+ '<div style="padding:10px;border-top:1px solid #eee;">'
					+ '<span>定时状态：</span>'
					+ '<input type="checkbox" id="cron-dialog-switch" lay-skin="switch" lay-text="启用|停用"'
					+ (currentEnabled ? ' checked' : '') + '>'
					+ '</div>',
				btn: ['确定', '取消'],
				yes: function(index) {
					var iframe = document.getElementById('cronEditorFrame');
					var cronValue = '';
					try {
						cronValue = iframe.contentWindow.document.getElementById('cron').value || '';
					} catch(e) {}
					var enabled = $('#cron-dialog-switch').is(':checked');
					pendingCronConfig = { cron: cronValue, enabled: enabled };
					layui.layer.close(index);
					layui.layer.msg('定时任务配置已暂存，请点击"保存"按钮生效');
				},
				success: function(layero, index) {
					layui.form.render();
				}
...
function saveCronConfigChain(id, done) {
	if (!pendingCronConfig) {
		done && done();
		return;
	}
	var config = pendingCronConfig;
	var afterCron = function() {
		if (config.enabled !== null && config.enabled !== undefined) {
			$.ajax({
				url: 'spider/' + (config.enabled ? 'start' : 'stop'),
				data: { id: id },
				success: function() {
					pendingCronConfig = null;
					done && done();
				},
				error: function() {
					layui.layer.msg('启用/停用设置失败');
					pendingCronConfig = null;
					done && done();
				}
			});
	...
	if (config.cron !== null && config.cron !== undefined) {
		$.ajax({
			url: 'spider/cron',
			data: { id: id, cron: config.cron },
			success: function() { afterCron(); },
			error: function() {
				layui.layer.msg('Cron 表达式保存失败');
				pendingCronConfig = null;
				done && done();
			}
		});
