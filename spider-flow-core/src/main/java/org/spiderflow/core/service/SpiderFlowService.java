...
public class SpiderFlowService extends ServiceImpl<SpiderFlowMapper, SpiderFlow> {
	...
	public boolean save(SpiderFlow spiderFlow){
		...
		if(StringUtils.isNotEmpty(spiderFlow.getId())){	//update 任务
			int rows = sfMapper.updateSpiderFlow(spiderFlow.getId(), spiderFlow.getName(), spiderFlow.getXml());
			if(rows > 0){
				...
				// ID 不存在，回退为 insert
				sfMapper.insertSpiderFlow(spiderFlow.getId(), spiderFlow.getName(), spiderFlow.getXml());
			}
		}else{//insert 任务
