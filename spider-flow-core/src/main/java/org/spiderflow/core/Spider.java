...
public class Spider {
	...
	public void runWithTest(SpiderNode root, SpiderContext context) {
		runWithTest(root, context, new HashMap<>());
	}

	/**
	 * 测试模式执行，支持传入初始变量（用于 iframe 嵌入时从 URL 参数注入）
	 */
	public void runWithTest(SpiderNode root, SpiderContext context, Map<String, Object> variables) {
		...
		//执行根节点（将外部传入的初始变量带入流程）
		executeRoot(root, context, variables == null ? new HashMap<>() : variables);
		//当爬虫任务执行完毕时,判断是否超过预期
