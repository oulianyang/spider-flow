...
public class FireFoxDriverProvider implements DriverProvider {
	...
	public String support() {
		// ${selenium.driver.firefox:null} 占位符缺失时注入的是字面量字符串 "null"（非 Java null），
		// 故需同时排除空串和字符串 "null"，避免把 "null" 当成驱动路径去 System.setProperty
		return StringUtils.isNotBlank(firefoxDriverPath) && !"null".equals(firefoxDriverPath) ? "firefox" : null;
	}
