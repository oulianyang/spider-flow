...
public class ChromeDriverProvider implements DriverProvider {
    ...
    public String support() {
        // ${selenium.driver.chrome:null} 占位符缺失时注入的是字面量字符串 "null"（非 Java null），
        // 故需同时排除空串和字符串 "null"，避免把 "null" 当成驱动路径去 System.setProperty
        return StringUtils.isNotBlank(chromeDriverPath) && !"null".equals(chromeDriverPath) ? "chrome" : null;
    }
    ...
    public WebDriver getWebDriver(SpiderNode node, String proxyStr) {
        ...
        //容器/共享内存受限环境必备：改用 /tmp 而非 /dev/shm，避免 tab 崩溃（page crash）
